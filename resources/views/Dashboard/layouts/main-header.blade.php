            ...
            <div class="nav nav-item  navbar-nav-right ml-auto">
                ...
                <div class="dropdown main-profile-menu nav nav-item nav-link">
                    ...
                    <div class="dropdown-menu">
                        ...
                        <a class="dropdown-item" href=""><i class="bx bx-user-circle"></i>{{__('Dashboard/main-sidebar-trans.profile')}}</a>
                        <a class="dropdown-item" href=""><i class="bx bx-cog"></i>{{__('Dashboard/main-sidebar-trans.edit_profile')}}</a>
                        @if(auth('web')->check())
                            <form method="POST" action="{{ route('logout.user') }}">
                                @elseif(auth('admin')->check())
                                    <form method="POST" action="{{ route('logout.admin') }}">
                                            ...
                                            <form method="POST" action="{{ route('logout.doctor') }}">
                                                @elseif(auth('ray_employee')->check())
                                                    <form method="POST" action="{{ route('logout.ray_employee') }}">
                                                        @elseif(auth('laboratories_Employee')->check())
                                                            <form method="POST"
                                                                  action="{{ route('logout.laboratories_employee') }}">
                                                                @else
                                                                    <form method="POST"
                                        ...
                                        this.closest('form').submit();"><i class="bx bx-log-out"></i>{{__('Dashboard/main-sidebar-trans.logout')}}</a>
                                                                    </form>
...
<script>
    ...
    var pusher = new Pusher('3fd9da89b2901f695a78', {
        cluster: 'mt1'
    });

    var channel = pusher.subscribe('invoices.{{auth()->user()->id}}');
    channel.bind('invoices-event', function(data) {
