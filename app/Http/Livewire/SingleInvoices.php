...
use App\Models\Appointment_book;
...
use App\Models\WebSite_Appointment;
use Livewire\Component;
use App\Events\Invoices;
...
use App\Models\single_invoice\single_invoice;
        ...
        if($this->type == 1){
            ...
            try {
                ...
                // في حالة الاضافة
                    ...
                    $appointment_info = WebSite::where('doctor_id', $this->doctor_id)->where('email', $patient->email)->where('type','مؤكد')->first();
                    if ($appointment_info) {
                        $appointment = WebSite::find($appointment_info->id);
                        $appointment->update([
                            'type' => 'منتهي'
                        ]);
                    }
